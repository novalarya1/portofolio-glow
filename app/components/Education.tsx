...
export default function Education() {
  return (
    <section className="py-24 px-6 md:px-12 bg-charcoal relative" id="education">
      <div className="max-w-4xl mx-auto">
        {/* Header */}
        <div className="text-center mb-16">
          <div className="inline-block px-3 py-1 rounded-full border border-neonBlue/20 bg-neonBlue/5 text-neonBlue text-xs font-medium tracking-wider uppercase mb-4">
            Learning Path
          </div>
          <h2 className="text-4xl font-bold text-white tracking-tight">Education Timeline</h2>
        </div>

        {/* Timeline Container */}
        <div className="relative">
          {/* Garis Tengah (Line) */}
          <div className="absolute left-4 md:left-1/2 transform md:-translate-x-1/2 top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-white/10 to-transparent" />
            ...
            {education.map((item, index) => (
              ...
              >
                ...
                <div className={`ml-12 md:ml-0 md:w-1/2 ${index % 2 === 0 ? "md:pr-16 text-left md:text-right" : "md:pl-16 text-left"}`}>
                  <div className="p-8 rounded-[2rem] bg-white/5 border border-white/5 hover:border-white/10 transition-all group overflow-hidden relative">
                    {/* Hover Glow */}
                    <div className="absolute inset-0 bg-white/[0.02] opacity-0 group-hover:opacity-100 transition-opacity" />
                    
                    <div className={`flex items-center gap-2 mb-3 text-sm font-medium ${index % 2 === 0 ? "md:justify-end" : "justify-start text-neonBlue"}`}>
                    ...
                    <p className="text-gray-300 font-medium mb-3 flex items-center gap-2 ${index % 2 === 0 ? 'md:justify-end' : ''}">
                       {item.institution}
                    </p>
                    
                    <div className={`flex items-center gap-2 text-xs text-gray-500 mb-4 ${index % 2 === 0 ? "md:justify-end" : ""}`}>
                       <MapPin size={12} />
